	...
	m_projects(projects)
{
	...
	m_errorIcon->setPixmap(QPixmap(":/cruisecontrol/resources/red-ball.png"));
	m_noConnectionIcon->setPixmap(QPixmap(":/cruisecontrol/resources/red-cross.png"));
	m_passIcon->setPixmap(QPixmap(":/cruisecontrol/resources/green-ball.png"));
...
QString	DisplayWidget::tooltipText() const
{
	...
	for (int i = 0 ; i < numProjects; i++) {
		QString color = m_projects->passed(i) ? "green" : "red";

		rc +="<tr>";
		rc += QString("<td><font color=\"%1\">%2</font></td>").arg(color).arg(m_projects->name(i));
		rc += QString("<td><font color=\"%1\">%2</font></td>").arg(color).arg(m_projects->date(i));
		rc +="</tr>\n";
	}
	rc +="</table>\n";
